...
{
    public static void InitDb(WebApplication app) // Void because nothing is being returned. Static so we don't have to use a constructor for DB Initializer class
    {
    ...
    private static void SeedData(AuctionDbContext context) // Static method used to seed data
    {
        context.Database.Migrate();

        if (context.Auctions.Any()) // Checks if there is already seed data that exists
        {
            Console.WriteLine("Already have data - no deed to seed");
            return;
        }

        var auctions = new List<Auction>() // Creates a new list of auctions
        {
            // auctions seed data goes here. Got data from CourseAssets folder
            	    // 1 Ford GT
            new Auction
            {
        ...
        context.AddRange(auctions); // AddRange - entity framework is going to keep the auctions that we add in memory

        context.SaveChanges(); // If we want to update our database we use SaveChanges
    }
}
